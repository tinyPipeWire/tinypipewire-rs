...
pub enum Error {
    ...
    InvalidArgument,
    /// Connecting to PipeWire, or asking it to create a stream, link or
    /// query, failed.
    ConnectFailed,
    ...
    /// was set, linking before starting, or unlinking with nothing linked.
    NotConfigured,
    /// The connected source disappeared, or could not provide the requested
    /// memory type.
    SourceUnavailable,
    /// The call came from inside one of the object's own callbacks, where it
    /// cannot run; make it again after the callback returns.
    InCallback,
    /// No node in the graph matches the target name or serial.
    NotFound,
    /// PipeWire did not answer, or a link did not negotiate, in time; the
    /// same call may succeed if retried.
    Timeout,
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let text = match self {
            ...
            Error::ConnectFailed => "PipeWire could not be reached or refused the request",
            ...
            Error::InCallback => "the call cannot be made from inside the object's own callback",
            Error::NotFound => "no node in the graph matches the target",
            Error::Timeout => "PipeWire did not answer in time",
